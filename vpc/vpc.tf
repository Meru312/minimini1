...
resource "aws_subnet" "mp_public_subnet1" {
  ...
  tags = {
    Name = "${var.project_name}-mp_public_subnet1"
  }
}
...
resource "aws_subnet" "mp_public_subnet2" {
  ...
  tags = {
    Name = "${var.project_name}-mp_public_subnet2"
  }
}
...
resource "aws_subnet" "mp_private_subnet1" {
  ...
  tags = {
    Name = "${var.project_name}-mp_private_subnet1"
  }
}
...
resource "aws_subnet" "mp_private_subnet2" {
  ...
  tags = {
    Name = "${var.project_name}-mp_private_subnet2"
  }
}
...
resource "aws_route_table" "mp_route_table_public" {
  ...
  tags = {
    Name = "${var.project_name}-mp_route_table_public"
  }
}
...
resource "aws_route_table" "mp_route_table_private" {
  ...
  tags = {
    Name = "${var.project_name}-mp_route_table_private"
  }
}
...
resource "aws_security_group" "mp_web_sg" {
  ...
  egress {
    ...
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]  # Allows all outbound traffic (can be restricted if needed)
  }
  tags = {
    Name = "${var.project_name}-web-sg"
  }
